...
DECLARE
  ...
  supabase_url TEXT := current_setting('request.headers', true)::json->>'x-forwarded-host';
BEGIN
  ...
  IF NEW.stock_actual < NEW.stock_minimo AND (OLD.stock_actual IS NULL OR OLD.stock_actual >= NEW.stock_minimo) THEN
    ...
    PERFORM net.http_post(
      url := 'https://zkqwrhdcryxfkeneiqqd.supabase.co/functions/v1/notify-low-stock-whatsapp',
...
DROP TRIGGER IF EXISTS check_low_stock_trigger ON productos;
